...
import { importContactsCsv, type ContactImportReport } from "@/server/actions/contacts-import";
    ...
    <div className="space-y-6">
      <section className="card p-6 max-w-3xl">
        ...
        <p className="text-xs text-midnight-500 mt-3">
          💡 Si la colonne <code className="px-1 bg-midnight-50 rounded">companyName</code> contient une entreprise inconnue, elle sera créée automatiquement (statut PROSPECT).
        </p>
      </section>
        ...
        <section className="card p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              onClick={() => start(async () => {
                try {
                  const r = await importContactsCsv(text);
                  setReport(r);
                  toast.success(`${r.created} créés · ${r.updated} mis à jour · ${r.errors.length} erreurs`);
      ...
      {report && (
        <section className="card p-5">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            ...
            <Stat label="Créés" value={report.created} tone="success" />
            <Stat label="Mis à jour" value={report.updated} tone="info" />
